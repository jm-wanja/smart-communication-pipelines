...
import { Box, Typography, Button, TextField, FormControl, InputLabel, Select, MenuItem, Grid, CircularProgress, Paper } from '@mui/material';
import { useNavigate } from 'react-router-dom';
import api from '../utils/api';

function AddDeal({ socket }) {
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                type="date"
                value={formData.projected_exchange_date}
                onChange={handleChange}
            ...
            <Grid item xs={12} sx={{ mt: 2 }}>
              <Button type="submit" variant="contained" color="primary" size="large" disabled={loading} sx={{ mr: 2 }}>
                {loading ? <CircularProgress size={24} /> : 'Create Deal'}
              </Button>
              <Button variant="outlined" onClick={() => navigate('/deals')} size="large" disabled={loading}>
                Cancel
              </Button>
            </Grid>
          </Grid>
